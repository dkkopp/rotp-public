...
public class CombatStackShip extends CombatStack
{
  ...
  @Override
  public boolean hostileTo(CombatStack st, StarSystem sys)
  {
    return st.isMonster() || empire.aggressiveWith(st.empire, sys);
  }

  @Override
  public CombatStack ward()
  {
    return ward;
  }
  ...
  @Override
  public boolean hasTeleporting()
  {
    return design.allowsTeleporting();
  }

  @Override
  public boolean canScan()
  {
    return design.allowsScanning();
  }
  ...
  @Override
  public float autoMissPct()
  {
    return displacementPct;
  }
  ...
  @Override
  public boolean canDamage(CombatStack target)
  {
    return estimatedKills(target) > 0;
  }

  @Override
  public float bombDamageMod()
  {
    return 0;
  }
    ...
    for (int i = 0; i < weapons.size(); i++)
    {
      ...
      else if (wpn.isMissileWeapon())
      {
        if (roundsRemaining[i] > 0)
        {
          ...
          if (distanceTo(0, MAX_Y) > tgt.distanceTo(0, MAX_Y))
          {
            targetBackOffRange = min(targetBackOffRange, tgt.distanceTo(0, MAX_Y));
          }
          if (distanceTo(MAX_X, 0) > tgt.distanceTo(MAX_X, 0))
          {
            targetBackOffRange = min(targetBackOffRange, tgt.distanceTo(MAX_X, 0));
          }
          if (distanceTo(MAX_X, MAX_Y) > tgt.distanceTo(MAX_X, MAX_Y))
          {
            targetBackOffRange = min(targetBackOffRange, tgt.distanceTo(MAX_X, MAX_Y));
          }
  ...
  {
    return design.wpnCount(i);
  }

  @Override
  public int shotsRemaining(int i)
  {
    return shotsRemaining[i];
  }
