...
public class StarSystem implements Base, Sprite, IMappedObject, Serializable
{
  ...
  @Override
  public boolean hasDisplayPanel()
  {
    return true;
  }
  ...
  {
    return concat("Star System: ", name());
  }
  ...
  {
    return concat(name, ":", str(planet().terrainSeed()));
  }

  public String ruinsKey()
  {
    return planet().ruinsKey();
  }
  ...
  {
    notes = s.length() <= 40 ? s : s.substring(0, 40);
  }
  ...
  public void addEvent(StarSystemEvent e)
  {
    events.add(e);
  }

  public List<StarSystemEvent> events()
  {
    return events;
  }
  ...
  public boolean piracy()
  {
    return piracy;
  }

  public void piracy(boolean b)
  {
    piracy = b;
  }
  ...
  public List<ShipFleet> exitingFleets()
  {
    return galaxy().ships.deployedFleets(id);
  }

  public List<ShipFleet> incomingFleets()
  {
    return galaxy().ships.incomingFleets(id);
  }
  ...
  {
    return isColonized() ? colony().population() : 0.0f;
  }
  ...
  ;
    public boolean hasBonusTechs()
  {
    return planet().hasBonusTechs();
  }
  ...
  public void inNebula(boolean b)
  {
    inNebula = b;
  }
  ...
  public boolean hasColonyForEmpire(Empire c)
  {
    return empire() == c;
  }

  public boolean hasOrbitingTransports()
  {
    return !orbitingTransports.isEmpty();
  }

  public boolean hasStargate(Empire e)
  {
    return isColonized() && colony().hasStargate(e);
  }
  ...
  public String getAttribute(String key)
  {
    ...
    {
      case "NAME":
        return empire().sv.name(id);
      case "POPULATION":
        return str(empire().sv.population(id));
      case "DELTA_POPULATION":
        return str(empire().sv.deltaPopulation(id));
      case "SIZE":
      ...
      case "PLANET_TYPE":
        return planet().type().name();
      case "NOTES":
        return notes();
      case "FACTORIES":
        return str(empire().sv.factories(id));
      case "DELTA_FACTORIES":
        return str(empire().sv.deltaFactories(id));
      case "WASTE":
        return str((int) colony().ecology().waste());
      case "INCOME":
        return str((int) colony().totalIncome());
      case "CAPACITY":
        return concat(str((int) (colony().currentProductionCapacity() * 100)), "%");
      case "RESERVE":
        return str((int) colony().reserveIncome());
      case "BASES":
        return str(empire().sv.bases(id));
      case "SHIPYARD":
        return colony().shipyardProject();
      case "DELTA_BASES":
        return str(empire().sv.deltaBases(id));
      case "SHIELD":
        return str(colony().defense().shieldLevel());
      case "TRANSPORT_TURNS":
        return str((int) Math.ceil(transportTimeTo(TARGET_SYSTEM)));
      case "RESOURCES":
        return text(empire().sv.view(id).resourceType());
    }
    return "";
  }
  ...
  @Override
  public IMappedObject source()
  {
    return this;
  }

  @Override
  public boolean persistOnClick()
  {
    return true;
  }
...
//            RotPUI.instance().selectDisplaySystemPanel(this);
  ...
  private boolean canShowDetail(GalaxyMapPanel map)
  {
    return map.parent().isClicked(this) && player().sv.isScouted(id);
  }
  ...
  private void drawSelection(Graphics2D g, GalaxyMapPanel map, Empire emp, int x, int y)
  {
    ...
    switch (shape)
    {
      case Empire.SHAPE_SQUARE:
        g.drawRect(x - r1, y - r1, r1 + r1, r1 + r1);
        break;
