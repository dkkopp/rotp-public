...
public class Precision
{
   ...
   * equal to 1. This value is an upper bound on the relative error due to rounding real numbers to
  ...
  /**
   * Compares two numbers given some amount of allowed error. Two float numbers are considered equal
   * if there are {@code (maxUlps - 1)} (or fewer) floating point numbers between them, i.e. two
   * adjacent floating point numbers are considered equal. Adapted from <a
   ...
   * y</li>
   * <li>&gt; 0 if !{@link #equals(double, double, int) equals(x, y, maxUlps)} &amp;&amp; x > y or
   * either argument is NaN</li></ul>
   */
  ...
  /**
   * Returns true if the arguments are both NaN, are equal, or are within the range of allowed error
   * (inclusive).
   *
   * @param x first value
   * @param y second value
   ...
   * difference between them is within the range of allowed error (inclusive). Returns {@code false}
  ...
  /**
   * Returns {@code true} if there is no double value strictly between the arguments or the relative
   * difference between them is less than or equal to the given tolerance. Returns {@code false} if
   ...
   * @param x Value to round.
   * @param scale Number of digits to the right of the decimal point.
   * @param roundingMode Rounding method as defined in {@link BigDecimal}.
   * @return the rounded value.
   * @since 1.1 (previously in {@code MathUtils}, moved as of version 3.0)
          ...
          throws MathArithmeticException, MathIllegalArgumentException
  {
    ...
    return (float) roundUnscaled(x * factor, sign, roundingMode) / factor;
  }
   ...
   * <pre><code>
   *   x + delta - x
   * </code></pre> is exactly machine-representable. This is useful when computing numerical
